...
public class TP02Q07 {
    public static void main(String[] args) {
        ...
        int contador[] = insertionSort(JogadorSelecionado);
        ...
        try{
            FileWriter fileWriter = new FileWriter("matricula_insercao.txt");
 ...
 public static int[] insertionSort(ArrayList<Jogador> insercao){
	...
	for(int i = 1; i < n; i++){
		Jogador tmp = insercao.get(i);
		int j = i - 1;
        comparacoes++;
        while(j >= 0 && insercao.get(j).getAnoNascimento() > tmp.getAnoNascimento()){

			insercao.set(j + 1, insercao.get(j));
			j--;
		}

        while(j >= 0 && insercao.get(j).getAnoNascimento() == tmp.getAnoNascimento() && insercao.get(j).getNome().compareTo(tmp.getNome()) > 0){
            insercao.set(j + 1, insercao.get(j));
			j--;
	        movimentacoes +=2;
		}
		insercao.set(j + 1, tmp);
		movimentacoes++;
	}

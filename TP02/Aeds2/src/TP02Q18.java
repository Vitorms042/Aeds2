...
public class TP02Q18 {
	public static void main(String[] args) {
        ...
        quickSortParcial(JogadorSelecionado, 0, JogadorSelecionado.size() - 1);
        int n = 10;

		if(n >= 0 && n < JogadorSelecionado.size()){
            List<Jogador> sub = JogadorSelecionado.subList(n, JogadorSelecionado.size());
            sub.clear();
        }
    ...
    public static void quickSortParcial(ArrayList<Jogador> array, int esq, int dir){
        int i = esq, j = dir;
        
        Jogador pivot = array.get((esq + dir) / 2);
        while(i <= j){
            while(compare(array.get(i), pivot) < 0){
                i++;
            }
            while(compare(array.get(j), pivot) > 0){
                j--;
            }
            if(i <= j){
                Jogador tmp = array.get(i);
    ...
    public static int compare(Jogador jg1, Jogador jg2){
        int res = jg1.getEstadoNascimento().compareToIgnoreCase(jg2.getEstadoNascimento());
        
        if(res == 0){
            res = jg1.getNome().compareToIgnoreCase(jg2.getNome());
        }
        return res;
    }
}
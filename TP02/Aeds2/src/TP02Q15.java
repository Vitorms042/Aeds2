...
public class TP02Q15 {
	...
	public static void main(String[] args) {
        ...
        selectionSortParcial(JogadorSelecionado);
        ...
        try{
            FileWriter fileWriter = new FileWriter("matricula_selectionParcial.txt");
            BufferedWriter arqWriter = new BufferedWriter(fileWriter);
            arqWriter.write("Matricula: 808664\tTempo: " +(tempoFim - tempoIni) / 1000d + "\tComparacoes: "+ comparacao + "\tMovimentacoes: " + movimentacoes);
            arqWriter.close();
        }catch(Exception e){
            System.out.println(e.getMessage() + "\n" + e.getLocalizedMessage());
        }
    ...
    public static void selectionSortParcial(ArrayList<Jogador> array){
        int n = array.size();
        int k = 10;
        for(int i = 0; i < k; i++){
            int menor = i;
            for(int j = (i + 1); j < n; j++){
                comparacao++;
                if(array.get(j).getNome().compareToIgnoreCase(array.get(menor).getNome()) < 0){
                    menor = j;
                }
            }
            swap(array, menor, i);

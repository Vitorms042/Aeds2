...
public class TP02Q03 {
    ...
    public static void main(String[] args) {
        ...
        ArrayList<String> JogadorSelecionado = new ArrayList<>();
		Scanner scanner = new Scanner(System.in);
        String id = scanner.nextLine();
        while (!id.equals("FIM")) {
            JogadorSelecionado.add(jogadores.get(Integer.parseInt(id)).getNome());
            id = scanner.nextLine();
        }
        ...
        while(!string.equals("FIM")){
            if(pesquisaSequencial(JogadorSelecionado, string)[0] == 1){
        ...
        try{
            ...
            arqWriter.write("Matricula: 808664\tTempo: " +(tempoFim - tempoIni) / 1000d + "\tComparacoes: "+ comparacoes);
            arqWriter.close();
        }catch(Exception e){
            System.out.println(e.getMessage() + "\n" + e.getLocalizedMessage());
        }
    ...
    public static int[] pesquisaSequencial(ArrayList<String> jogador, String string) {
        int  resposta = 0;
        for (int i = 0; i < jogador.size(); i++) {
            comparacoes++;
            if (jogador.get(i).equals(string)) {
                resposta = 1;
                i = jogador.size();
            }
        }
        return new int[] {resposta};
    }
}